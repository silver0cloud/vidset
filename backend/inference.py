...
class TTSInferenceEngine:
    ...
    def _ensure_loaded(self) -> None:
        if self._loaded:
            return
        self._load_model()
        # Print actual preprocess signature once for debugging
        from f5_tts.infer.utils_infer import preprocess_ref_audio_text
        import inspect
        print(f"  preprocess_ref_audio_text signature: {inspect.signature(preprocess_ref_audio_text)}")
        self._loaded = True
    ...
    def _synthesize(self, text: str) -> np.ndarray:
        ...
        # Inspect actual installed signature to handle F5-TTS version differences
        sig    = inspect.signature(preprocess_ref_audio_text)
        params = sig.parameters

        kwargs = {"show_info": lambda x: None}
        if "device" in params:
