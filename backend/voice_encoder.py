...
NOTE: With the real F5-TTS integration, this module is used for
Quality Control only — NOT by the inference pipeline. F5-TTS uses
zero-shot voice cloning (ref_audio + ref_text), so no speaker embedding
is passed to the model during inference or training.

This module remains valuable for:
    - Detecting inconsistent takes before training (saves training time)
    - Flagging re-record candidates to the user
...
# resemblyzer is optional — pulls in webrtcvad which fails to build on Windows.
# If not installed, voice consistency checks are skipped gracefully.
try:
    from resemblyzer import VoiceEncoder as ResemblyzerEncoder, preprocess_wav
...
MIN_TAKES_REQUIRED  = 5         # Minimum takes to build a reliable embedding
...
class VoiceEncoder:
    ...
    def encode_from_dataset(
        ...
        Returns a profile with ready_for_training=False if resemblyzer unavailable.
        """
        if not RESEMBLYZER_AVAILABLE:
            print("  resemblyzer not installed — skipping voice consistency check.")
